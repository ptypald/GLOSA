...
static void printsol(node_t initial, int it) {
	...
	if (OUTPUT) {
		...
		char buf_dir[0x200];
		snprintf(buf_dir, sizeof(buf_dir), "results_dddp/x0=%.1f-v0=%.1f/", P.x0, P.v0);
		...
		char buf[0x200];
		snprintf(buf, sizeof(buf), "results_dddp/x0=%.1f-v0=%.1f/(%02d) x0=%.1f - v0=%.1f - delta=%.1f - du=%.3f.txt", P.x0, P.v0, it + 1, P.x0, P.v0, DELTA_V, du);
		f = fopen(buf,"w");
...
int main(int argc, char **argv) {
	...
	for (int xi = 0; xi < 1; xi++) {
		for (int vi = 0; vi < 1; vi++) {
			...
			// 1. calculate initial trajectory (deterministic solution of pessimistic case)
			// negSpeedInitCon = 0;
