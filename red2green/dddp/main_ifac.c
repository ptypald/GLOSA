...
static void printsol(node_t initial, int it) {
	...
	if (OUTPUT) {
		char buf[0x100];
		snprintf(buf, sizeof(buf), "outputs/(%02d) x0=%.1f - v0=%.1f - delta=%.1f - du=%.3f.txt", it + 1, X0, V0, DELTA_V, du);
		f = fopen(buf,"w");
...
int main(int argc, char **argv) {
	...
	FILE *f_idm;
	char buf[0x100];
	snprintf(buf, sizeof(buf), "outputs/IDM - x0=%.1f - v0=%.1f.txt", X0, V0);
	f_idm = fopen(buf,"w");
